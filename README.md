...
1. We need a proxy server to communicate with the Tiger Graph REST API. You can use https://github.com/canbax/tiger-api to run a proxy server. The server makes HTTP request to Tiger Graph REST API and passes responses to the client-side.
2. Run `npm i` and run `npm run ng s` to run angular in development mode.
3. Open http://localhost:4200 if everything is OK, `Data>Sample Data` should bring some data from your Tiger Graph Database. 
